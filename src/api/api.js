...
export const filmsApi = {
    ...
    getFilms(filters, page){
        return instance.get(`movie?${filters.selectedTypeOfMovies.map((item)=>`&typeNumber=${item}&`).join('')}${filters.genre.length ? filters.genre.map((item)=>`&genres.name=${item.value}&`).join(''):''}&genres.name=!документальный&genres.name=!ток-шоу&genres.name=!реальное ТВ&genres.name=!короткометражка&genres.name=!церемония&genres.name=!игра&genres.name=!концерт&genres.name=!для взрослых&year=${filters.year[0]}-${filters.year[1]}&rating.kp=${filters.rating[0]}-${filters.rating[1]}&sortField[]=year&sortType[]=${filters.sortByRelease}&limit=40&page=${page}&field=poster&search=!null&countries.name=!Корея Южная&countries.name=!Китай&countries.name=!Таиланд&countries.name=!Индия&countries.name=!Япония&countries.name=!Тайвань&countries.name=!Турция`)
